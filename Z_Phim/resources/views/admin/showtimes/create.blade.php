...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-white">Thêm suất chiếu mới</h1>
        <p class="mt-2 text-slate-400">Tạo suất chiếu cho phim và phòng mà bạn quản lý.</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.showtimes.store') }}">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select id="cinema_id" name="cinema_id" required class="w-full rounded-2xl border border-slate-700 bg-slate-900/70 px-4 py-3 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        @foreach($cinemas as $cinema)
                            <option value="{{ $cinema->id }}" data-rooms='@json($cinema->rooms)' {{ old('cinema_id') == $cinema->id ? 'selected' : '' }}>{{ $cinema->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    ...
                    <button type="submit" class="rounded-2xl bg-indigo-500 px-6 py-3 text-sm font-semibold text-white hover:bg-indigo-400 transition">Tạo suất chiếu</button>
                </div>
            </div>
        </form>
    </div>
